...
>(({ side = "left", variant = "sidebar", collapsible = "none", className, children, ...props }, ref) => {
  return (
    <div
      ref={ref}
      className={cn("flex h-full w-[--sidebar-width] flex-col bg-white border-r border-gray-200", className)}
      {...props}
    >
      {children}
    </div>
...
const SidebarGroupLabel = React.forwardRef<HTMLDivElement, React.ComponentProps<"div"> & { asChild?: boolean }>(
  ({ className, asChild = false, ...props }, ref) => {
    ...
    return (
      <Comp ref={ref} className={cn("flex h-8 shrink-0 items-center px-4 text-xs font-medium", className)} {...props} />
    )
  },
)
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm outline-none transition-colors hover:bg-gray-100 focus-visible:ring-2 active:bg-gray-100 disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-purple-100 data-[active=true]:text-purple-600",
  {
    variants: {
      variant: {
        default: "hover:bg-gray-100",
        outline: "bg-background shadow-sm hover:bg-gray-100",
      },
      size: {
        default: "h-8 text-sm",
        sm: "h-7 text-xs",
